...
falls under feature perturbation. On a contact/grasp \emph{act vs.\ abort}
proxy with redundant sensors, a residual router that \emph{switches
channels} under perturbation recovers utility (\NumPertRouterUtil{} vs.\
deployed-channel gating \NumPertIidUtil, \NumDualNSeeds{} seeds) and
beats abort-all (\NumPertAbortUtil). This is an analysis-plus-system
result, not a new calibrator and not a physical robot.
\end{abstract}
...
one blob. They do not route different act/abort policies for sensor
disagreement vs.\ workspace selection.
...
\item \textbf{Numpy grasp proxy, not a robot.} Dual-regime numbers are
from \texttt{exp08\_dual\_regime} (HistGradientBoosting, five seeds).
No MuJoCo, no DexNet.
\item \textbf{Search scope.} Novelty is relative to
\texttt{paper/verified.bib}.
\end{itemize}

\section{Dual-regime act/abort}
...
task is whether that split changes \emph{what the agent does}.

We use a contact/grasp proxy (\texttt{src/dualregime/}): success is
physics on true pose, yaw, and force. The \emph{deployed} policy sees
only encoder, motor current, and an unused appearance channel. Camera
pose and a force gauge are a watchdog. Perturbation biases encoder and
motor; labels stay on true physics. Selection restricts true $g_x$ to a
far band (pairs kept). A residual detector (encoder$-$camera,
motor$-$gauge) labels regimes. Under perturbation the policy
\emph{switches channels} and applies a failure-controlling gate
$P(\mathrm{act}\mid y{=}0)\approx\alpha$ fitted on the watchdog
features. Under i.i.d./selection it keeps the deployed-channel gate.
Abort-all and always-watchdog are ablations.

Figure~\ref{fig:dual} and \texttt{results/summary\_dual.json}:
under encoder/motor bias, router mean utility is \NumPertRouterUtil{}
vs.\ deployed i.i.d.\ gating \NumPertIidUtil{} vs.\ abort-all
\NumPertAbortUtil{} (\NumDualNSeeds{} seeds). False-confident-act rate
\NumPertRouterFcr{} vs.\ \NumPertIidFcr; success recall
\NumPertRouterRec. On in-support i.i.d.\ tests the router does not
pay a large tax (\NumIidRouterUtil{} vs.\ \NumIidIidUtil). Far-band
selection is hard for every mode; we do not claim the i.i.d.\ gate
transfers out of support. This is a structural proxy, not hardware
\cite{luo2021conformal}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figures/png/dual_regime_utility.png}
\caption{Mean act/abort utility by test split and policy (five seeds).
Sensor perturbation is not the same as workspace selection.}
\label{fig:dual}
\end{figure}
...
dependable \emph{decision} policy when the deployed sensors are the
ones that moved. A residual router that switches to watchdog channels
recovers utility under that perturbation and is not equivalent to
abort-all. Pairing-preserving selection is a different mechanism.
The HGB exception and the killed H2/H3 hypotheses remain part of the
tabular measurement.
