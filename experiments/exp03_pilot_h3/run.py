"""P2 pilot for H3: unweighted vs oracle-weighted conformal coverage vs shift strength."""
                    ...
                    out.append(
                        {
                            ...
                            "shift": {
                                "kind": "gaussian_feature_shift",
                                "strength": s,
                                "n_features": cfg["n_shift_features"],
                            },
                            "alpha": cfg["alpha"],
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp03_pilot_h3.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp03_pilot_h3"
