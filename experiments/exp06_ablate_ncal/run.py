"""P3 ablation: calibration-set size (killed primary H2)."""
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp06_ablate_ncal.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp06_ablate_ncal"
