"""P3 control: shift trailing (less designed-to-be-predictive) features."""
                ...
                out.append(
                    {
                        ...
                        "shift": {
                            ...
                            "cols": cfg["shift_cols_by_dataset"][ds],
                        },
                        "alpha": cfg["alpha"],
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp07_noise_feature_control.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp07_noise_feature_control"
