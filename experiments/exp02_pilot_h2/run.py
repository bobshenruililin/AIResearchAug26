"""P2 pilot for H2: isotonic vs temperature as a function of n_cal under shift."""
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp02_pilot_h2.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp02_pilot_h2"
