"""P2 pilot for H1: does i.i.d. post-hoc calibration transfer under shift?"""
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp01_pilot_h1.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp01_pilot_h1"
