"""P3 main grid for H1 (fixed shift columns, 5 seeds, 4 datasets)."""
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(ROOT / "results" / "exp04_main_h1.json"))
    args = p.parse_args()
    cfg = yaml.safe_load(Path(args.config).read_text())
    try:
        payload = run_grid(cells_from(cfg))
        payload["experiment"] = "exp04_main_h1"
